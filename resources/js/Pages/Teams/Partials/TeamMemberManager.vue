...
import DialogModal from "@/Components/Modals/ModalDialog.vue"
...
const managingRoleFor = ref<UserData | null>(null)
const confirmingLeavingTeam = ref<boolean>(false)
const teamMemberBeingRemoved = ref<UserData | null>(null)
const confirmingTeamMemberBeingRemoved = ref<boolean>(false)
...
const cancelTeamInvitation = (invitation: TeamInvitationData) => {
    router.delete(route("team-invitations.destroy", invitation), {
        preserveScroll: true,
    })
}

const manageRole = (teamMember: UserData) => {
...
const leaveTeam = () => {
    leaveTeamForm.delete(route("team-members.destroy", [props.team, usePage<Props>().props.user]))
}

const confirmTeamMemberRemoval = (teamMember: UserData) => {
    teamMemberBeingRemoved.value = teamMember
    confirmingTeamMemberBeingRemoved.value = true
}
...
const removeTeamMember = () => {
    removeTeamMemberForm.delete(route("team-members.destroy", [props.team, teamMemberBeingRemoved.value]), {
        ...
        preserveState: true,
        onSuccess: () => {
            confirmingTeamMemberBeingRemoved.value = false
            teamMemberBeingRemoved.value = null
        },
    })
}

const displayableRole = (role: string | undefined) => {
    return props.availableRoles.find((r: RoleData) => r.key === role)?.name
}
</script>
...
<template>
    <div>
        ...
        <ConfirmationModal
            :show="confirmingTeamMemberBeingRemoved"
            @close="confirmingTeamMemberBeingRemoved = false">
            ...
            <template #footer>
                <SecondaryButton @click="confirmingTeamMemberBeingRemoved = false"> Cancel </SecondaryButton>
